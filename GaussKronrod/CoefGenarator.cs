...
namespace GaussKronrod {
    public static class CoefGenarator {
        static void AbscissaWeights1(int n, int m, double eps, double coef2, bool even, double[] b, ref double x, ref double w) {
            double ai, b0 = 0, b1, b2, d0, d1, d2 = 0, delta, dif, f, fd = 0, yy;
            int i, iter, k, ka;
        ...
        public static (double[] x, double[] w1, double[] w2) Coef(int n, double eps) {
            ...
            //
            bb = Math.Sin(Math.PI / (2 * (an + an + 1)));
            x1 = Math.Sqrt(1 - bb * bb);
            s = 2 * bb * x1;
            c = Math.Sqrt(1 - s * s);
            ...
            //
            for (k = 1; k <= n; k += 2) {
                AbscissaWeights1(n, m, eps, coef2, even, b, ref xx, ref w1[k - 1]);
                w2[k - 1] = 0;
                ...
                //
                AbscissaWeights2(n, m, eps, coef2, even, b, ref xx, ref w1[k], ref w2[k]);
            ...
            if (even) {
                xx = 0;
                AbscissaWeights1(n, m, eps, coef2, even, b, ref xx, ref w1[n]);
                w2[n] = 0;
                x[n] = xx;
            }
